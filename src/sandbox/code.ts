...
function start() {
    runtime.exposeApi({
        ...
        insertMath: async (data: { imageData: ArrayBuffer; width: number; height: number; position?: { x: number, y: number } }) => {
            ...
            try {
                ...
                const imageContainer = editor.createImageContainer(bitmap, { initialSize: { width: data.width, height: data.height } });

                if (data.position) {
                    imageContainer.translation = { x: data.position.x, y: data.position.y };
                } else {
                    // Center it
                    imageContainer.translation = { x: artboard.width / 2 - data.width / 2, y: artboard.height / 2 - data.height / 2 };
                }
