...
export async function parseXLSXFile(file: File): Promise<ParseResult> {
  ...
  // Extract header
  const columns = (rawData[0] as (string | number)[]).map(c => (c === null || c === undefined ? "" : String(c).trim()));
 ...
 */
function convertRowsToTyped(rawRows: string[][], columnInfo: ColumnInfo[]): (string | number)[][] {
