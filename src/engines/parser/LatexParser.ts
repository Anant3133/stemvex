...
export class LatexParser {
  ...
  static parse(input: string): Token[] {
    ...
    return tokens.reduce((acc: Token[], token) => {
      ...
      const equationRegex =
        /(?:[a-zA-Z0-9\(\)'′\[\]\{\}\\_]+\s*[=≈≡→]\s*(?:(?![A-Z][a-z]+|\b(and|the|this|that|then|where|allows|linearity)\b)[\w\d\+\-\*\/\^\(\)\{\}\[\]_\\∑∫∏∂∇√∞!]+(?:\s+(?![A-Z][a-z]{2,}|\b(and|the|this|that|then|where|allows|linearity)\b)[\w\d\+\-\*\/\^\(\)\{\}\[\]_\\∑∫∏∂∇√∞!]+)*))(?=[.,;:?!]?(?:\s|$))/g;

      // 2. Complex Structure Glue: e.g. "\sum a_i" or "∫ f(x) dx"
      const complexRegex =
        /(?:[∑∫∏∂∇√]\s*(?:(?![A-Z][a-z]+)[\w\d\+\-\*\/\^\(\)\{\}\[\]_\\=]+(?:\s+(?![A-Z][a-z]{2,})[\w\d\+\-\*\/\^\(\)\{\}\[\]_\\=]+)*))(?=[.,;:?!]?(?:\s|$))/g;

      // 3. Standalone Terms
      const termsRegex =
        /([a-zA-Z0-9]+\^[a-zA-Z0-9\(\)\{\}\-]+|[a-zA-Z0-9]+_[a-zA-Z0-9\(\)\{\}\-]+|\b(?:sin|cos|tan|log|ln|lim|sum|int|sqrt)\b\s*[\(\[].*?[\)\]])/g;
